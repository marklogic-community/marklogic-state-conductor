...
assertions.push(
  test.assertEqual(8, flowDocuments.length, 'Flow files are present')
);

const flowNames = sc.getFlowNames().sort();
assertions.push(
  test.assertEqual(8, flowNames.length, "flowNames count"),
  ...
  test.assertEqual('wait-flow', flowNames[7])
);
