...
/**
 * Given a state conductor stateMachine, validate against the
 * MarkLogic implementation of Amazon State Language
 *
 * @param {*} stateMachine
 * @returns
 */
function validateStateMachineFile(stateMachine) {
  try {
    xdmp.jsonValidateNode(stateMachine, schema);
    return true;
  } catch (err) {
    return false;
  }
