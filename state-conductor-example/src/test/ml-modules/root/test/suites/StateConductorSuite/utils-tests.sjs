...
assertions.push(
  test.assertEqual(
    ...
    JSON.stringify(scLib.materializeReferencePath('$', context))
  ),
  ...
  test.assertEqual(
    ...
    JSON.stringify(scLib.materializeReferencePath('$.name', context))
  ),
  test.assertEqual(
    'bar',
...
assertions.push(
  test.assertEqual(
    JSON.stringify(context.arr[0]),
    JSON.stringify(resp.arr),
    'xpath returns the first node of the array'
  ),
  test.assertEqual(
    JSON.stringify(context.arr[0]),
    JSON.stringify(resp.arr1),
    'xpath uses 1 based array origin'
  ),
  test.assertEqual(
    JSON.stringify(context.arr[1]),
