...
//queries the executions database for the tde view
const tdeView = fn.head(
  xdmp.invokeFunction(
    () => {
      return tde.getView('StateConductor', 'ExecutionsProvenance');
    },
...
const columns = tdeView.columns.reduce(
  (acc, curr) => ((acc[curr.column.name] = curr.column.scalarType), acc),
  {}
);
...
assertions.push(
  test.assertEqual('dayTimeDuration', columns.executionTime, 'columns executionTime')
);
assertions.push(test.assertEqual('dateTime', columns.date, 'columns date'));

//queries the executions database to see have there are documents that match the query
const sql = xdmp.invokeFunction(
  () => {
    return xdmp.sql('select * from ExecutionsProvenance limit 10', 'map');
  },
  { database: xdmp.database(sc.STATE_CONDUCTOR_EXECUTIONS_DB) }
);
